...
export async function GET(_req: NextRequest, ctx: Ctx): Promise<Response> {
  ...
  const authz = await requirePermission("profissionais.ver", { requestId, resource: "clinic_event_type_resources" });
...
export async function PUT(req: NextRequest, ctx: Ctx): Promise<Response> {
  ...
  const requestId = randomUUID();
  const authz = await requirePermission("profissionais.gerenciar", { requestId, resource: "clinic_event_type_resources" });
  if (!authz.ok) return authz.response;
