...
| eslint.config.mjs, supabase/baseline.sql (de novo), supabase/migrations/MANIFEST.md (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), lib/i18n/dicionario.ts (de novo) | Prontuário F0 (9016): `no-console` vira erro no código clínico (lib/clinic, app/api/v1/clinic, components/clinic, app/app/atendimentos); apêndice e linha da 9016; `fn_clinic_definir_prontuario` na allowlist de definer; espanhol das permissões clínicas | (este PR) |
| lib/navigation/catalogo.ts (de novo), lib/api/errors.ts (de novo), lib/audit/actions.ts (de novo), .github/workflows/e2e.yml (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo) | Prontuário F1 (9017): porta "Meus atendimentos" (`/app/atendimentos`, sem sidebar); códigos `prontuario_desligado`, `atendimento_em_andamento`, `atendimento_ja_encerrado`; ações `clinic.atendimento_*`; spec clinic-atendimento-fila; tabelas clínicas em PROVA_PROPRIA | (este PR) |
| app/app/contacts/[id]/_client.tsx, app/app/contacts/[id]/page.tsx, lib/api/errors.ts (de novo), lib/audit/actions.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F2 (9018/9019): aba "Prontuário" no detalhe do paciente (só com `prontuario.ver`); códigos `prontuario_imutavel`, `requisitos_pendentes`; ações `clinic.registro_criado`, `clinic.adendo_criado`, `clinic.prontuario_visto`; 5 tabelas clínicas em PROVA_PROPRIA; funções `fn_clinic_salvar_formulario/salvar_evolucao/adicionar_adendo` na allowlist | (este PR) |
| lib/navigation/catalogo.ts (de novo), lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), .github/workflows/e2e.yml (de novo) | Prontuário F3 (9020): porta "Modelos clínicos" em Configurações (`modelos_clinicos.gerenciar`); ações `clinic.modelo_*` e `clinic.requisitos_definidos`; `clinic_requisitos_finalizacao` em PROVA_PROPRIA; funções do editor e dos requisitos na allowlist; spec clinic-modelos-clinicos | (este PR) |
| app/app/contacts/[id]/_client.tsx (de novo), lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F4 (9021): aba "Planos" no detalhe do paciente (só com `planos.ver`); ações `clinic.plano_*`; condutas, planos e sessões em PROVA_PROPRIA; funções de conduta e plano na allowlist; e2e com conduta e plano | (este PR) |
| lib/event-log/register-handlers.ts, lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F5 (9022): consumidor `clinic-estoque-consumo.v1` de `clinic.procedimento_confirmado` (porta de estoque; hoje `SemEstoque`, só marca como lido); ação `clinic.procedimento_anulado`; procedimentos e insumos em PROVA_PROPRIA; funções na allowlist; e2e com procedimento e evento | (este PR) |
| lib/auth/public-paths.ts, app/app/contacts/[id]/_client.tsx (de novo), lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), .github/workflows/e2e.yml (de novo) | Prontuário F6 (9023): `/termo/<token>` e `/api/v1/publico/termos/<token>` públicos (token de 43 caracteres ancorado; o token é a identidade, só o hash no banco, uso único, prazo, limite por IP); aba "Documentos" no paciente (`documentos.ver`); ações `clinic.documento_*`; 5 tabelas em PROVA_PROPRIA; funções na allowlist; spec clinic-documentos | (este PR) |
| docs/threat-model.md, app/app/contacts/[id]/_client.tsx (de novo), lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F7 (9024): bucket privado `clinical-files` registrado no threat model; aba "Fotos e anexos" no paciente (`prontuario.ver`; a RLS filtra por tipo); ações `clinic.anexo_*` e `clinic.foto_divulgacao`; `clinic_anexos` em PROVA_PROPRIA; funções na allowlist; e2e com foto | (este PR) |
| lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F8 (9025): página `/imprimir/prontuario/[contactId]` (fora do layout do app, `prontuario.exportar`); ações `clinic.atendimento_reaberto` e `clinic.prontuario_exportado`; `fn_clinic_reabrir_atendimento` na allowlist; cerca `clinic-auditoria-sem-conteudo-clinico` | (este PR) |
| lib/audit/actions.ts (de novo), lib/i18n/dicionario.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F9 (9026): ações `clinic.cabecalho_alterado` e `clinic.atendimento_anulado`; `fn_clinic_cabecalho_salvar` e `fn_clinic_anular_atendimento` na allowlist; `clinic_prontuarios`/`clinic_prontuario_alteracoes` com prova própria; e2e do cabeçalho e dos filtros | #35 |
| lib/i18n/dicionario.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo), tests/invariants/clinic-acesso-clinico.test.ts, tests/invariants/clinic-anexos-e-fotos.test.ts | Prontuário F10 (9027): correções das revisões; `fn_member_permissions` redefinida (suporte sem `documentos.*`); `fn_clinic_anexo_divulgar` na allowlist; testes das fases ajustados à marcação com canais | (este PR) |
