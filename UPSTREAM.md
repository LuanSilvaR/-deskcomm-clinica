...
| app/app/contacts/[id]/_client.tsx, lib/api/errors.ts (de novo), lib/audit/actions.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo) | Prontuário F2 (9017/9018): aba "Prontuário" no detalhe do paciente (só com `prontuario.ver`); códigos `prontuario_imutavel`, `requisitos_pendentes`; ações `clinic.registro_criado`, `clinic.adendo_criado`, `clinic.prontuario_visto`; 5 tabelas clínicas em PROVA_PROPRIA; funções `fn_clinic_salvar_formulario/salvar_evolucao/adicionar_adendo` na allowlist | (este PR) |
