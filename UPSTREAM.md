...
| eslint.config.mjs, supabase/baseline.sql (de novo), supabase/migrations/MANIFEST.md (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), lib/i18n/dicionario.ts (de novo) | Prontuário F0 (9015): `no-console` vira erro no código clínico (lib/clinic, app/api/v1/clinic, components/clinic, app/app/atendimentos); apêndice e linha da 9015; `fn_clinic_definir_prontuario` na allowlist de definer; espanhol das permissões clínicas | (este PR) |
| lib/navigation/catalogo.ts (de novo), lib/api/errors.ts (de novo), lib/audit/actions.ts (de novo), .github/workflows/e2e.yml (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo) | Prontuário F1 (9016): porta "Meus atendimentos" (`/app/atendimentos`, sem sidebar); códigos `prontuario_desligado`, `atendimento_em_andamento`, `atendimento_ja_encerrado`; ações `clinic.atendimento_*`; spec clinic-atendimento-fila; tabelas clínicas em PROVA_PROPRIA | (este PR) |
| app/app/contacts/[id]/_client.tsx, app/app/contacts/[id]/page.tsx, lib/api/errors.ts (de novo), lib/audit/actions.ts (de novo), tests/invariants/rls-completude-varredura.test.ts (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), tests/e2e/clinic-atendimento-fila.spec.ts (de novo) | Prontuário F2 (9017/9018): aba "Prontuário" no detalhe do paciente (só com `prontuario.ver`); códigos `prontuario_imutavel`, `requisitos_pendentes`; ações `clinic.registro_criado`, `clinic.adendo_criado`, `clinic.prontuario_visto`; 5 tabelas clínicas em PROVA_PROPRIA; funções `fn_clinic_salvar_formulario/salvar_evolucao/adicionar_adendo` na allowlist | (este PR) |
