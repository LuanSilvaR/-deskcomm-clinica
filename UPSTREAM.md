...
| app/app/agenda/_client.tsx (de novo), lib/navigation/catalogo.ts (de novo) | Botão e porta "Agenda do dia" (só com `clinic.agenda_do_dia` ligada, 9014) | (este PR) |
| app/api/v1/agenda/vinculos/route.ts (de novo) | A busca de paciente (nome/telefone/CPF/nascimento) passa a usar `filtrarContatosPelaBusca` (lib/clinic/pacientes/busca-no-banco.ts), a mesma da Agenda do dia; comportamento igual | (este PR) |
| lib/ui/icons.ts | Ícones DoorOpen, Stethoscope, UserMinus, XCircle (selos de status da Agenda do dia) | (este PR) |
| lib/i18n/dicionario.ts (de novo), .github/workflows/e2e.yml (de novo) | Espanhol da Agenda do dia; spec clinic-agenda-do-dia | (este PR) |
