...
| lib/navigation/{catalogo,interface,registry}.ts, lib/auth/types.ts, lib/ui/icons.ts, components/shell/{Sidebar,CommandPalette}.tsx, app/app/layout.tsx, app/app/page.tsx | Menu da clínica (9014): porta `/app/inicio` (fim do catálogo, sem sidebar); `homeDaInterface` com casa preferida opcional; `ActiveOrg.menu_clinica`; Sidebar troca só o miolo por `NavDaClinica` com a flag ligada; ⌘K rotula pelo módulo; ícones Wrench/CurrencyCircleDollar/Percent. Desligado = igual ao de antes (só o ⌘K ganha "Início") | (este PR) |
| supabase/baseline.sql (de novo), supabase/migrations/MANIFEST.md (de novo), tests/invariants/hardening-definer-varredura.test.ts (de novo), lib/i18n/dicionario.ts (de novo), .github/workflows/e2e.yml (de novo), app/api/v1/clinic/config/route.ts | Migration 9014 (`fn_clinic_definir_menu_clinica`); opção `menu_clinica` no config clinic; espanhol dos módulos; spec clinic-menu-da-clinica | (este PR) |
...
| lib/ui/icons.ts (de novo), lib/i18n/dicionario.ts (de novo), .github/workflows/e2e.yml (de novo) | Início da recepção: ícone UserPlus, espanhol, spec clinic-inicio-da-recepcao | (este PR) |
...
| package.json, pnpm-lock.yaml | Editor do POP: @tiptap/react, @tiptap/pm, @tiptap/starter-kit, @tiptap/extension-table, @tiptap/extensions (3.31.3, MIT) | (este PR) |
| lib/i18n/dicionario.ts (de novo), .github/workflows/e2e.yml (de novo) | POP (9015): espanhol do editor, versões e histórico; spec clinic-pop | (este PR) |
| lib/i18n/dicionario.ts (de novo) | POP (9015): impressão em PDF A4 — "Imprimir" em espanhol (o renderizador lib/documentos/ é código do fork) | (este PR) |
